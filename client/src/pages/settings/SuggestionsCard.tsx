...
export default function SuggestionsCard({
  ...
  const persistedProviderRef = useRef(settings.suggestionProvider);
  const toastRef = useRef(toast);
  onSettingsRef.current = onSettings;
  providerChoiceRef.current = providerChoice;
  persistedProviderRef.current = settings.suggestionProvider;
  ...
  // Provider changes can normalize the effective model on the server (for
  // example OpenRouter's vendor-qualified default versus a custom endpoint).
  // Keep the controlled inputs aligned with every returned settings snapshot
  // so a later blur cannot write the previous provider's model back.
  useEffect(() => {
    setAiModel(settings.suggestionModel);
  ...
  useEffect(() => {
    ...
    pollerRef.current = createCodexDevicePoller({
      ...
      onState: (state) => {
        ...
        setCopied(false);
        const providerToPersist = providerPersistenceForCodexTransition(
          state.phase,
          providerChoiceRef.current === 'codex',
        );
        if (providerToPersist) {
          ...
          setProviderChoice(persistedProviderRef.current);
          setCodexStatus({
            connected: false,
  ...
  const disconnectCodex = async () => {
    ...
    try {
      ...
      // The provider connection is part of live-autopilot readiness. Re-emit
      // settings so the parent invalidates the readiness summary immediately.
      onSettings(await instanceSettings.get());
      setDeviceState(null);
      setCodexStatus({ connected: false, state: 'disconnected', reconnectRequired: false });
