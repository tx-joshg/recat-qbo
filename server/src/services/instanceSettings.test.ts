...
vi.mock('../lib/prisma.js', () => ({
  prisma: {
    ...
    $transaction: vi.fn(async (callback) =>
      callback({ appConfig: mocks.appConfig, company: mocks.company }),
    ),
  },
}));
...
beforeEach(() => {
  ...
  mocks.company.updateMany.mockResolvedValue({ count: 0 });
...
describe('suggestion model setting precedence', () => {
  ...
  it('vendor-qualifies the default model when OpenRouter is selected', async () => {
  ...
  it('preserves an explicit OpenRouter model selection', async () => {
    mocks.suggestionProvider = 'openrouter';
    mocks.suggestionModel = 'anthropic/claude-sonnet-4';

    await expect(getInstanceSettings()).resolves.toMatchObject({
      suggestionModel: 'anthropic/claude-sonnet-4',
    });
  });
});
...
describe('Codex provider settings', () => {
  ...
  it('stores Codex model plaintext and returns it in the masked settings DTO', async () => {
    ...
    expect(mocks.company.updateMany).toHaveBeenCalledWith({
      where: {
