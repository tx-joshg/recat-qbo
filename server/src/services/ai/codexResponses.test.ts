...
describe('buildCodexRequest', () => {
  ...
  it('adds strict function tools, continuation items, and a structured final format', () => {
    expect(
      buildCodexRequest({
        model: 'gpt-5.6-luna',
...
describe('Codex Responses transport', () => {
  ...
  it('parses fragmented and parallel function calls with replayable linkage', async () => {
    const response = streamResponse([
      event({
    ...
    await expect(parseCodexTurnSse(response)).resolves.toEqual({
      text: '',
      functionCalls: [
        {
          itemId: 'item-1',
  ...
  it('counts larger arguments.done replacements toward the aggregate output cap', async () => {
    const chunks: string[] = [];
    for (let index = 0; index < 9; index += 1) {
      ...
      chunks.push(
        ...
        event({
          ...
          output_index: index,
          item_id: itemId,
          arguments: `{"value":"${'x'.repeat(240_000)}"}`,
        }),
      );
    }
    chunks.push(event({ type: 'response.completed' }));

    await expect(parseCodexTurnSse(streamResponse(chunks))).rejects.toThrow(
      /output was too large/i,
    );
  });

  it('counts larger output_item.done replacements toward the aggregate output cap', async () => {
    const chunks: string[] = [];
    for (let index = 0; index < 9; index += 1) {
      const itemId = `item-${index}`;
      chunks.push(
  ...
  it('keeps structured final text as a replayable assistant output item', async () => {
    const text = JSON.stringify({ kind: 'skip', reasonCode: 'NO_EVIDENCE', rationale: 'Unknown.' });
    const result = await parseCodexTurnSse(
      streamResponse([
  ...
  it('rejects incomplete function-call output', async () => {
    await expect(
      parseCodexTurnSse(
        streamResponse([
          event({
    ...
    ).rejects.toThrow(/incomplete function call/i);
  });
});
