  ...
  return {
    ...
    async releaseFlow({
      ...
      await database.$executeRawUnsafe(
        `UPDATE ai_codex_device_flows
         SET state = $2::"CodexDeviceFlowState",
    ...
    async completeFlow({ id, encryptedCredential, reconciliationToken }) {
      return database.$transaction(async (client) => {
        ...
        await client.$executeRawUnsafe(
          `UPDATE "Company"
           SET "agentReconcileToken" = $1
           WHERE "autopilotMode" IN ('shadow', 'live')`,
          reconciliationToken,
        );
        return true;
      });
...
export function createCodexAuth({
  ...
  /**
   * Stable, non-secret account generation for long-running workers. This is a
   * local credential read: it never refreshes OAuth tokens or calls OpenAI.
   */
  async function getAccountGeneration(): Promise<string | null> {
    const encryptedCredential = await store.getCredential();
    if (!encryptedCredential) return null;
    try {
      const credential = decryptedCredential(encryptedCredential, decryptFn);
      return credential.state === 'connected' && credential.accountId
        ? sha256Hex(credential.accountId)
        : null;
    } catch {
      return null;
    }
...
export const getCodexAccountGeneration = defaultService.getAccountGeneration;
