...
class MemoryStore implements CodexStore {
  ...
  async claimFlow({
    ...
    if (
      (flow.state === 'polling' || flow.state === 'authorized') &&
      flow.updatedAt > staleBefore
    ) {
...
describe('device authorization', () => {
  ...
  it('preserves an authorized exchange across a transient failure and restart', async () => {
    ...
    await expect(
      createCodexAuth({ store, fetchFn: retryFetch, now: () => now }).pollDeviceFlow({
        flowId,
        ...owner,
      }),
  ...
  it('does not exchange an authorized code twice while the first exchange is active', async () => {
    const now = Date.now();
    const store = new MemoryStore();
    const flow: CodexDeviceFlowRecord = {
      id: 'flow-authorized',
      ...owner,
      deviceAuthIdEnc: null,
      userCodeEnc: null,
      authorizationCodeEnc: encrypt('auth-code'),
      codeVerifierEnc: encrypt('verifier'),
      intervalMs: 1000,
    ...
    await expect(auth.pollDeviceFlow({ flowId: flow.id, ...owner })).resolves.toEqual({
      status: 'pending',
      retryAfterMs: 1000,
    });
    expect(fetchFn).not.toHaveBeenCalled();
    expect(store.flows.get(flow.id)?.state).toBe('authorized');
  });
});

describe('credential refresh and database locking', () => {
  it('reads a stable hashed account generation without refreshing OAuth', async () => {
    const store = new MemoryStore();
    seedCredential(store, { expiresAt: 0, accountId: 'acct-stable' });
    const fetchFn = vi.fn<typeof fetch>();
    const auth = createCodexAuth({ store, fetchFn });

    await expect(auth.getAccountGeneration()).resolves.toBe(sha256Hex('acct-stable'));
    ...
    await expect(auth.getAccountGeneration()).resolves.toBe(sha256Hex('acct-stable'));
    seedCredential(store, { accountId: 'acct-other' });
    await expect(auth.getAccountGeneration()).resolves.toBe(sha256Hex('acct-other'));
    expect(fetchFn).not.toHaveBeenCalled();
  });
  ...
  it('casts dynamic device-flow states to the Postgres enum', async () => {
    ...
    expect(executions[0]?.sql).toMatch(/state = \$2::"CodexDeviceFlowState"/);
    expect(executions[0]?.values[1]).toBe('pending');
  });

  it('marks enabled companies for durable reconciliation when authorization completes', async () => {
    const executions: Array<{ sql: string; values: unknown[] }> = [];
    const tx = {
      $queryRawUnsafe: vi.fn(async (sql: string) =>
        /ai_codex_device_flows/i.test(sql) ? [{ state: 'authorized' }] : [],
      ),
      $executeRawUnsafe: vi.fn(async (sql: string, ...values: unknown[]) => {
        executions.push({ sql, values });
        return 1;
    ...
    await expect(
      store.completeFlow({
        ...
        reconciliationToken: 'generation-1',
      }),
    ).resolves.toBe(true);
    ...
    expect(
      executions.some(
        ({ sql, values }) =>
          /UPDATE "Company"[\s\S]*"agentReconcileToken"/i.test(sql) &&
          values[0] === 'generation-1',
      ),
    ).toBe(true);
  });
});
